...
class RpcValidationError extends Error {
  /**
   * The RPC bridge classifies by code: a coded error is expected control flow
   * (terse, at warn), an uncoded one is an unexpected bug (at error, with a
   * stack). Bad params are the former, so they must cross the bridge coded.
   */
