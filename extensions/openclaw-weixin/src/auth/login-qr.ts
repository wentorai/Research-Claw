...
/**
 * Default session key used when no accountId is provided.
 * OC gateway's web.login.wait handler only forwards { timeoutMs, accountId },
 * stripping the sessionKey returned by web.login.start. Using a deterministic
 * default key instead of randomUUID() ensures loginWithQrWait can find the
 * active login even without the sessionKey being forwarded.
 */
export const WEIXIN_DEFAULT_SESSION_KEY = "__weixin_default__";
...
export async function startWeixinLoginWithQr(opts: {
  ...
  const sessionKey = opts.accountId || WEIXIN_DEFAULT_SESSION_KEY;
