...
- Tools: `periph_list` (registered devices + camera bridge status),
  `periph_camera_snap` (grab a frame via the dashboard bridge),
  `periph_observe` (record a check/note observation).
- **Device monitor contract:** for recurring device checks, call `periph_list`
  first, then `monitor_create(source_type='device', target=<device id from
  periph_list>)`. Target MUST be the registered device id (uuid) — never a
  browser mediaDeviceId or a URL.
- Templates are system-generated by device kind: camera → snap frame + visual
  check; audio-recorder (Plaud) → fetch transcripts + digest notes.
