# R table template: clustered_se.R for econ-tables-cn
...
# expected keywords: sandwich lmtest vcovCL
# vc <- sandwich::vcovCL(lm(y ~ x, data = df), cluster = ~ firm_id)
# lmtest::coeftest(lm(y ~ x, data = df), vcov = vc)
