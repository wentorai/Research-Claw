 ...
 *   → POST /rc/upload  (destination `periph/<deviceId>`)  via uploadFileToWorkspace
...
/** Payload of `plugin.rc.periph.captureRequest` (bridge.ts:129). */
...
export default function PeriphCaptureListener() {
  ...
  useEffect(() => {
    ...
    // Hydrate the device list on (re)connect so the peripherals panel is fresh.
    // Mirrors stores/gateway.ts:116-118 (loadMonitors on connect). Without this,
    // loadDevices() had zero production callers and the store stayed empty.
    void usePeripheralsStore.getState().loadDevices();
...
async function handleCaptureRequest(
  ...
  try {
    ...
    const uploaded = await uploadFileToWorkspace(file, `periph/${deviceId}`);
    path = uploaded.path;
  } catch (err) {
