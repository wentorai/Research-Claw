...
const COST_COPY: Record<string, CopySpec[]> = {
  en: [
    ...
    },
    {
      ...
      approved:
        'Deep review unavailable: {{reason}}. The deterministic safety gate (quick checks + dangerous-command policy) is still running.',
      required: [/\{\{reason\}\}/, /(deterministic|safety)[^.;]*\bstill\b[^.;]*\brun/i],
      forbidden: [
        /disabled/i,
        /turned off/i,
        /\bnot\s+running\b/i,
        new RegExp(`(deterministic|safety)[^.;]*${EN_NEG}[^.;]*\\b(run|apply|block)`, 'i'),
      ],
      rejects: [
        'Deep review unavailable: {{reason}}. The deterministic safety gate is not running.',
        'Deep review unavailable: {{reason}}. The deterministic safety gate no longer runs.',
        "Deep review unavailable: {{reason}}. The deterministic safety gate won't run until a reviewer is configured.",
        'Supervision disabled: {{reason}}. The deterministic safety gate is still running.',
        'Deep review is turned off: {{reason}}. The deterministic safety gate is still running.',
        'Deep review unavailable. The deterministic safety gate is still running.', // no {{reason}}
        'Deep review unavailable: {{reason}}.', // the reassurance dropped entirely
        'Deep review unavailable: {{reason}}. The deterministic safety gate is still running. The reviewer is not running.',
        'Deep review unavailable: {{reason}}. The deterministic safety gate is still running. Safety checks cannot apply.',
      ],
    },
    {
      // Same promise on the audit panel — the two surfaces must not disagree.
      key: 'supervisor.reviewerUnavailable',
      approved: 'Deep review unavailable, deterministic safety gate still running: {{reason}}',
      required: [/\{\{reason\}\}/, /(deterministic|safety)[^.;]*\bstill\b[^.;]*\brun/i],
      forbidden: [
        /disabled/i,
        /turned off/i,
      ...
      rejects: [
        'Deep review unavailable, deterministic safety gate not running: {{reason}}',
        'Deep review unavailable, deterministic safety gate no longer running: {{reason}}',
        'Supervision turned off: {{reason}}',
        'Deep review disabled, deterministic safety gate still running: {{reason}}',
        'Deep review unavailable, deterministic safety gate still running',
        'Deep review unavailable: {{reason}}',
        'Deep review unavailable, deterministic safety gate still running: {{reason}}. The reviewer is turned off.',
        'Deep review unavailable, deterministic safety gate still running: {{reason}}. The reviewer is not running.',
        'Deep review unavailable, deterministic safety gate still running: {{reason}}. Safety checks cannot apply.',
      ],
    },
  ...
  'zh-CN': [
    ...
    {
      key: 'settings.supervisorReviewerUnavailable',
      approved: '深审不可用：{{reason}}。确定性安全闸（快速检查 + 危险命令策略）仍在运行。',
      required: [/\{\{reason\}\}/, /(确定性|安全闸)[^，,。；;]*仍[^，,。；;]*(运行|工作|生效)/],
      forbidden: [
        /已关闭/,
        /已停用/,
        /(未|不)(再)?(运行|生效)/,
        new RegExp(`(确定性|安全闸)[^，,。；;]*${ZH_NEG}[^，,。；;]*(运行|工作|生效|拦截)`),
      ],
      rejects: [
        '深审不可用：{{reason}}。确定性安全闸也已停用。',
        '深审不可用：{{reason}}。确定性安全闸不再运行。',
        '深审不可用：{{reason}}。确定性安全闸无法生效。',
        '监督已关闭：{{reason}}。确定性安全闸仍在运行。',
        '深审不可用。确定性安全闸仍在运行。', // 缺 {{reason}}
        '深审不可用：{{reason}}。', // 安抚语句被整段删掉
        '深审不可用：{{reason}}。确定性安全闸仍在运行。reviewer 已停用。',
        '深审不可用：{{reason}}。确定性安全闸仍在运行。reviewer 未运行。',
        '深审不可用：{{reason}}。确定性安全闸仍在运行。安全闸无法工作。',
      ],
    },
    {
      key: 'supervisor.reviewerUnavailable',
      approved: '深审不可用，确定性安全闸仍在运行：{{reason}}',
      required: [/\{\{reason\}\}/, /(确定性|安全闸)[^，,。；;]*仍[^，,。；;]*(运行|工作|生效)/],
      forbidden: [
        /已关闭/,
        /已停用/,
      ...
      rejects: [
        '深审不可用，确定性安全闸不再运行：{{reason}}',
        '深审不可用，确定性安全闸已停用：{{reason}}',
        '监督已关闭：{{reason}}',
        '深审不可用，确定性安全闸仍在运行',
        '深审不可用：{{reason}}',
        '深审不可用，确定性安全闸仍在运行：{{reason}}。reviewer 已关闭。',
        '深审不可用，确定性安全闸仍在运行：{{reason}}。reviewer 已停用。',
        '深审不可用，确定性安全闸仍在运行：{{reason}}。reviewer 未运行。',
        '深审不可用，确定性安全闸仍在运行：{{reason}}。安全闸无法工作。',
      ],
    },
